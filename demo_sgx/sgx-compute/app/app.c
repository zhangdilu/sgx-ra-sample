...
int testDigitalEnvelope(int symbol)
{
   ...
   //../app/app --decrypt --enclave-path `pwd`/../enclave/enclave.signed.so --statefile sealeddata.bin --encrypted-aes-key e_aes.bin cipher.txt
    ...
    if (symbol == 1)
    {
        ...
        opt_enclave_path = "../../../Enclave/Enclave.signed.so";
        opt_statefile = "../../sealeddata.bin";
        opt_aes_key_file = "../../encrypt/e_aes.bin";
        opt_input_file = "../../encrypt/cipher.txt";
    }
    ...
    //printf("%d",enclave_get_buffer_sizes());
    printf("this is success status");
    printf("%d",success_status);
